...
function makeAuditRoutes(config) {
	...
	return [{
		...
		handler: (req, res) => {
			...
			const sessionId = parseQueryParam(url, "session") ?? "";
			audit(cwd, detail, sessionId === "" ? void 0 : config.agents?.get(sessionId), sessionId, resolveHostLocale(parseQueryParam(url, "lang")), parseQueryParam(url, "fresh") === "1").then((report) => json(res, 200, {
				ok: true,
				report
			}), (error) => json(res, 500, {
				ok: false,

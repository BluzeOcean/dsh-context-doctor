...
async function scanSkillCatalog(skillList, signal) {
	...
	return {
		...
		duplicateDescriptions: modelVisible.map((s) => ({
			name: s.name,
...
function revealInOSExplorer(folder) {
	...
	try {
		...
		child.on("error", () => { /* 缺少打开器 / 权限不足：静默 */ });
		child.unref();
	} catch { /* spawn 同步失败：不影响面板其余交互 */ }
}
...
function makeAuditRoutes(config) {
	...
	return [{
		...
		handler: (req, res) => {
			...
			audit(cwd, detail, sessionId === "" ? void 0 : config.agents?.get(sessionId), sessionId, resolveHostLocale(parseQueryParam(url, "lang")), forceFresh).then((report) => json(res, 200, {
				ok: true,
				report
			}), (error) => json(res, 500, {
				ok: false,

...
import { REFRESH_CSS, REFRESH_STYLE_ID, ageSeconds, refreshView } from "./refresh.js";
...
function CheckIcon() {
    return _jsx("svg", { width: "13", height: "13", viewBox: "0 0 24 24", fill: "none", "aria-hidden": "true", children: _jsx("path", { d: "M5 12.5l4.2 4.2L19 7", stroke: "currentColor", strokeWidth: "2.4", strokeLinecap: "round", strokeLinejoin: "round" }) });
}
...
export function ContextAuditRing(props) {
    ...
    const refreshStart = useRef(null);
    const flashTimer = useRef(null);
    const [elapsed, setElapsed] = useState(0);
    const refresh = useCallback((forceFresh = false, settle = false) => {
    ...
    const revealFile = useCallback((relativePath) => {
        if (report === null)
            return;
        const lang = document.documentElement.lang.toLowerCase().startsWith('zh') ? 'zh' : 'en';
    ...
    useEffect(() => {
        if (document.getElementById(REFRESH_STYLE_ID) !== null)
            return undefined;
    ...
    const refreshPhase = state.state === 'loading' ? 'loading' : 'settled';
    const refreshState = refreshView(refreshPhase, flash, elapsed, t);
    return _jsxs("span", { ref: dockRef, "data-context-doctor": true, "data-cd-scope": styleScope, style: dockStyle, children: [_jsx("button", { type: "button", onClick: () => setOpen(value => !value), title: `${t('cd.title')} · ${status}`, "aria-label": `${t('cd.title')} · ${status}`, "aria-expanded": open, "aria-controls": panelId, style: { ...triggerStyle, color: accent }, children: _jsx(PulseIcon, { size: 16 }) }), open && _jsxs("section", { id: panelId, role: "dialog", "aria-label": t('cd.title'), style: panelStyle, children: [_jsxs("header", { style: headStyle, children: [_jsx("span", { style: eyebrowStyle, children: t('cd.title') }), _jsxs("span", { style: { ...statusStyle, color: accent }, children: [_jsx("span", { "aria-hidden": "true", style: { ...statusDotStyle, background: accent } }), status] })] }), state.state === 'error' && _jsxs("p", { style: errorStyle, children: [t('cd.error'), ": ", state.error] }), report === null && state.state !== 'error'
                        ...
                        : report !== null && _jsxs(_Fragment, { children: [_jsxs("div", { style: gaugeStyle, children: [_jsxs("div", { style: readStyle, children: [_jsx("strong", { style: readValueStyle, children: formatTokens(resident) }), _jsx("span", { style: readUnitStyle, children: t('cd.residentUnit') }), _jsxs("span", { style: readPercentStyle, children: [Math.round(percent * 100), "% / ", formatTokens(FULL_SCALE)] })] }), _jsxs("div", { style: railStyle, role: "img", "aria-label": `${formatTokens(resident)} / ${formatTokens(FULL_SCALE)}`, children: [_jsx("span", { style: railTrackStyle, children: segments.filter(segment => segment.tokens > 0).map(segment => _jsx("span", { style: {
                                        ...
                                        return _jsxs("li", { style: { listStyle: 'none' }, children: [_jsxs("button", { type: "button", disabled: !canExpand, onClick: () => setExpanded(current => current === segment.key ? null : segment.key), "aria-expanded": isOpen, title: canExpand ? (isOpen ? t('cd.collapse') : t('cd.expand')) : t('cd.noDetail'), style: { ...rowStyle, cursor: canExpand ? 'pointer' : 'default' }, children: [_jsx("span", { "aria-hidden": "true", style: {
                                                            ...
                                                            } }), _jsxs("span", { style: { minWidth: 0 }, children: [_jsx("span", { style: { ...rowLabelStyle, color: canExpand ? TONE.text : TONE.muted }, children: segment.label }), _jsx("span", { style: rowSubStyle, children: segment.sub })] }), _jsx("span", { style: rowValueStyle, children: formatTokens(segment.tokens) }), _jsxs("span", { style: rowShareStyle, children: [Math.round(share * 100), "%"] })] }), isOpen && segment.detail !== null && _jsxs("div", { style: detailStyle, children: [_jsx("span", { style: detailTitleStyle, children: segment.detail.title }), segment.detail.rows.slice(0, DETAIL_LIMIT).map(row => row.path !== undefined
                                                            ? _jsxs("button", { type: "button", onClick: () => revealFile(row.path), title: `${row.path} — ${t('cd.reveal')}`, style: { ...detailRowStyle, background: 'transparent', border: 0, color: 'inherit', font: 'inherit', textAlign: 'left', cursor: 'pointer' }, children: [_jsx("span", { style: { ...detailNameStyle, cursor: 'pointer', color: TONE.blue, textDecoration: 'underline', textDecorationColor: 'rgba(124, 155, 255, 0.45)', textUnderlineOffset: 2 }, children: row.name }), _jsx("span", { style: detailValueStyle, children: formatTokens(row.tokens) })] }, row.name)
                                                            : _jsxs("span", { style: detailRowStyle, title: row.name, children: [_jsx("span", { style: detailNameStyle, children: row.name }), _jsx("span", { style: detailValueStyle, children: formatTokens(row.tokens) })] }, row.name)), segment.detail.rows.length > DETAIL_LIMIT
                                            ...
                                            }) })] })] }), _jsxs("footer", { style: footerStyle, children: [_jsx("span", { style: updatedStyle, children: t('cd.updated', { when: updated }) }), _jsxs("button", { type: "button", "data-cd-refresh": true, "data-pressed": pressed ? 'true' : 'false', "data-cd-spin": refreshState.spinning ? 'true' : 'false', "data-cd-flash": refreshState.flash ? 'true' : 'false', onClick: () => refresh(true, true), onMouseDown: () => setPressed(true), onMouseUp: () => setPressed(false), onMouseLeave: () => setPressed(false), disabled: state.state === 'loading', title: refreshState.elapsed > 0 ? `${refreshState.label} ${refreshState.elapsed}s` : refreshState.label, "aria-label": refreshState.elapsed > 0 ? `${refreshState.label} ${refreshState.elapsed}s` : refreshState.label, style: { ...refreshButtonStyle, color: refreshState.flash ? TONE.mint : TONE.blue }, children: [refreshState.flash
                                        ? _jsx(CheckIcon, {})
                                        : _jsx("span", { "aria-hidden": "true", style: refreshState.elapsed > 0 ? { display: 'inline-flex' } : undefined, children: _jsx(RefreshIcon, {}) }), _jsx("span", { style: refreshLabelStyle, children: refreshState.label })] })] })] })] });
}
...
const refreshButtonStyle = { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 8px 4px 7px', color: TONE.blue, background: 'transparent', border: `1px solid ${TONE.border}`, borderRadius: 7, cursor: 'pointer', font: 'inherit', fontSize: 12, fontWeight: 500, lineHeight: 1 };
const refreshLabelStyle = { fontVariantNumeric: 'tabular-nums' };
